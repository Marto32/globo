...
parser.add_argument(
    "--username",
    type=str,
    help="The gmail address from which the emails will be sent (xxx@gmail.com)",
    required=True,
)
parser.add_argument(
    "--app_password",
    type=str,
    help="Your gmail app password to sign into the sender account.",
    required=True,
)
parser.add_argument(
    "--recipients",
    type=str,
    help="A comma separated list of one or more recipient email addresses.",
    required=True,
)
parser.add_argument(
    "--todoist",
    type=bool,
    help="If workouts should be sent to Todoist instead of gmail. If true, TODOIST_API_TOKEN environment variable is expected to be set.",
    required=False,
    default=False,
)

CURRENT_WORKOUT = workout_program.AttiaLongevity
...
if __name__ == "__main__":
    ...
    if today in CURRENT_WORKOUT.keys():
        ...
        if args.todoist:
            ...
            todoist.Todoist.create(os.environ.get("TODOIST_API_TOKEN")).addTask(
